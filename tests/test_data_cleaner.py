...
class TestDataCleaner(unittest.TestCase):
    ...
    def test_drop_invalid_rows_removes_rows_with_missing_values(self):
        ...
        # (usar self.assertEqual para comparar .isna().sum() con 0 - comparación simple de enteros, unittest es suficiente)
        self.assertEqual(resultado[["name","age"]].isna().sum().sum(),0,'No deberia haver valores faltantes')
        # - Verificar que el DataFrame resultante tiene menos filas que el original (usar self.assertLess con 
        # len() - comparación simple de enteros, unittest es suficiente)
        self.assertLess(len(resultado),len(df),'el df resultante deberia tener menos filas que el inicial')
    ...
    def test_trim_strings_strips_whitespace_without_changing_other_columns(self):
        ...
        #  (usar self.assertEqual para comparar valores específicos como strings individuales - unittest es suficiente para strings)
        self.assertEqual(df["name"].iloc[0]," Alice ", "el df inicial no debe ser modificado")
        # - Verificar que en el DataFrame resultante los valores de "name" 
        # no tienen espacios al inicio/final (usar self.assertEqual para comparar valores 
        # específicos como strings individuales - unittest es suficiente)
        self.assertEqual(resultado["name"].iloc[0],"Alice")
        # - Verificar que las columnas no especificadas (ej: "city") permanecen sin cambios 
        # (si comparas Series completas, usar pandas.testing.assert_series_equal() ya que maneja mejor los índices 
        # y tipos de Pandas; si comparas valores individuales, self.assertEqual es suficiente)
    ...
    def test_remove_outliers_iqr_removes_extreme_values(self):
        ...
        resultado=cleaner.remove_outliers_iqr(df,'age',factor=0.5)
        ...
        self.assertNotIn(120,resultado['age'].values,"120 deberia ser eliminado")
        # - Verificar que al menos uno de los valores no extremos (25 o 35) permanece en el resultado 
        # (usar self.assertIn para verificar que está presente)
